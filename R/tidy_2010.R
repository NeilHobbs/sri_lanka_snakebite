...
snakes = snakes2010_A  #update year for each extraction. Should really be done as a loop
...
snakes_tidy = df%>% #update snakes year
  mutate(Year = 2010)%>% #update year
  rename(Count = row)%>% 
  rename(Hospital = snakes....3)

write.csv(snakes_tidy, ".\\snakes2010.csv") #update year
