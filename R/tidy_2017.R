...
snakes = snakes2017_A  #update year for each extraction. Should really be done as a loop
...
snakes_tidy = df%>% #update snakes year
  mutate(Year = 2017)%>% #update year
  rename(Count = row)%>% 
  rename(Hospital = snakes....3)

write.csv(snakes_tidy, ".\\snakes2017.csv") #update year
