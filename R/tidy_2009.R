...
snakes = snakes2009_A  #update year for each extraction. Should really be done as a loop
...
snakes_tidy = df%>% #update snakes year
  mutate(Year = 2009)%>% #update year
  rename(Count = row)%>% 
  rename(Hospital = snakes....3)

write.csv(snakes_tidy, ".\\snakes2009.csv") #update year
